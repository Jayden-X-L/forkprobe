...
interface ToolResponse {
  status: 'selected' | 'ready' | 'awaiting_verdict' | 'no_verdict'
  reportPath?: string
  logPath?: string
  winner?: string
  winnerName?: string
...
function workspacePath(parent: Agent): string {
  return parent.session.header.cwd || process.cwd()
}

function resolveWorkspaceFile(workspace: string, requested: string | undefined, fallback: string): string {
...
function candidatePrompt(task: string, skill: PreparedSkill): string {
  return [
...
function judgePrompt(task: string, results: CandidateResult[], rubric?: string): string {
...
async function startSubagent(
  ...
  label: string,
  signal: AbortSignal,
): Promise<{ output: string; latencySeconds: number; error?: string }> {
  const provider = ctx.subagents.getProvider(config.provider)
  if (!provider) throw new Error(`ForkProbe subagent provider "${config.provider}" is not available`)
...
async function findLatestVerdictLog(workspace: string): Promise<string> {
  ...
  const candidates = entries
    ...
    .sort()
    .reverse()
  if (!candidates.length) throw new Error('no ForkProbe logs exist in this workspace')

  const latest = candidates[0]
  for (const candidate of candidates) {
    try {
      const log = await readJson(candidate)
      if ((log.verdict as Record<string, unknown> | null)?.winner) return candidate
    } catch {
...
async function waitForVerdict(
  ...
  while (Date.now() < deadline && !signal.aborted) {
    try {
      ...
      const verdict = log.verdict as Record<string, unknown> | null
      if (verdict?.winner) {
...
function responseSchema() {
  return {
    ...
    properties: {
      ...
      reportPath: { type: 'string' as const },
      logPath: { type: 'string' as const },
      winner: { type: 'string' as const },
      winnerName: { type: 'string' as const },
      verdictType: { type: 'string' as const },
      handoffText: { type: 'string' as const },
      selectedOutput: { type: 'string' as const },
      judgeWinner: { type: 'string' as const },
      message: { type: 'string' as const, required: true },
    },
  } as const
}

function renderResponse(_args: unknown, value: ToolResponse): ContentBlock[] {
  const lines = [value.message]
  if (value.reportPath) lines.push(`Report: ${value.reportPath}`)
  if (value.winnerName) lines.push(`User-selected winner: ${value.winnerName}`)
  if (value.handoffText) lines.push(`Continuation handoff: ${value.handoffText}`)
  if (value.selectedOutput) lines.push(`Selected output:\n${value.selectedOutput}`)
...
export function apply(ctx: Context, config: Config): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec): Promise<ToolResponse> {
      const parent = exec.agent
      if (!parent) throw new Error('forkprobe_compare requires a calling DSH Agent')
      ...
      const reportPath = resolveWorkspaceFile(
        workspace,
        args.output_path,
        `forkprobe-dsh-${new Date().toISOString().replace(/[:.]/g, '')}-report.html`,
      )
      ...
      try {
        ...
        const skills = prepared.skills as PreparedSkill[]
        ...
        const results = await Promise.all(skills.map(async skill => {
          const run = await startSubagent(
            ctx,
            config,
            parent,
            candidatePrompt(args.task, skill),
            `ForkProbe: ${skill.name}`,
            exec.signal,
          )
          return {
            ...skill,
            ...
            token_count_method: 'estimated_visible_context' as const,
            provider_tokens_used: 0 as const,
          }
        }))

        let judge: Record<string, unknown> = { enabled: false }
        if (args.judge !== false) {
          const judgeRun = await startSubagent(
            ctx,
            config,
            parent,
            judgePrompt(args.task, results, args.judge_rubric),
            'ForkProbe AI judge',
            exec.signal,
          )
          judge = {
        ...
        const finalized = JSON.parse(finalizedText) as Record<string, unknown>
        const ready: ToolResponse = {
          status: 'ready',
        ...
        if (args.wait_for_verdict === false || !finalized.verdict_connected || !ready.logPath) return ready
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec): Promise<ToolResponse> {
      const parent = exec.agent
      if (!parent) throw new Error('forkprobe_resume requires a calling DSH Agent')
      const workspace = workspacePath(parent)
      ...
      try {
        ...
        const results = Array.isArray(bundle.results) ? bundle.results as Record<string, unknown>[] : []
        const winner = String(verdict.winner)
